...
import * as Aria from '@tiger/utils/dom/aria'

import TrapFocus, { FOCUSABLE_CHILDREN } from '../trap-focus'
import type { ITrapFocusElement } from '../trap-focus'

const isVisibleMock = jest.spyOn(Aria, 'isVisible').mockImplementation(() => true)
...
describe('v-trap-focus', () => {
	test('should fetch all focusable element', () => {
      ...
      `)
		expect((wrapper.element as ITrapFocusElement)[FOCUSABLE_CHILDREN].length).toBe(1)
	})
    ...
    `)
		expect((wrapper.element as ITrapFocusElement)[FOCUSABLE_CHILDREN].length).toBe(5)
	})
